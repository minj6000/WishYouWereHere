    ...
    public class ReminderItem : MonoBehaviour
    {
        ...
        public void InitializeWindow(ModalWindowManager modal, TMP_InputField title, HorizontalSelector type, HorizontalSelector minute, HorizontalSelector hour, HorizontalSelector meridiem)
        {
            ...
            modal.onConfirm.AddListener(delegate 
            {
                ...
                for (int i = 0; i < manager.timeManager.timedEvents.Count; i++)
                {
                    if (manager.timeManager.timedEvents[i].eventID == reminderID)
                    {
                        if (meridiem.index == 0) { manager.timeManager.timedEvents[i].meridiemFormat = DateAndTimeManager.DefaultShortTime.AM; }
                        else { manager.timeManager.timedEvents[i].meridiemFormat = DateAndTimeManager.DefaultShortTime.PM; }

                        if (type.index == 0) { manager.timeManager.timedEvents[i].eventType = DateAndTimeManager.TimedEventType.Once; }
                        else if (type.index == 1) { manager.timeManager.timedEvents[i].eventType = DateAndTimeManager.TimedEventType.Daily; }

                        manager.timeManager.timedEvents[i].eventTitle = title.text;
                        manager.timeManager.timedEvents[i].eventHour = hour.index + 1;
                        manager.timeManager.timedEvents[i].eventMinute = minute.index;
