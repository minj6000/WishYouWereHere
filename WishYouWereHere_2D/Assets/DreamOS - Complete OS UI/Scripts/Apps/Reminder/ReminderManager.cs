...
namespace Michsky.DreamOS
{
    public class ReminderManager : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            ...
            // Set as target reminder manager
            timeManager.reminderManager = this;
            ...
            for (int i = 0; i < reminderLimit; i++)
            {
                if (DreamOSDataManager.ContainsJsonKey(dataCat, "ReminderItem#" + i.ToString() + "_IsEnabled"))
                {
                    ...
                    // Add items
                    activeItems.Add(rItem);
                    timeManager.timedEvents.Add(tEvent);
                }
            }
        }
        ...
        public void EnableReminder(string itemID, bool value, bool updateSwitch = true)
        {
            ...
            for (int i = 0; i < timeManager.timedEvents.Count; i++)
            {
                if (timeManager.timedEvents[i].eventID == itemID)
                {
                    timeManager.timedEvents[i].isEnabled = value;
                    break;
                }
            }
