...
namespace Michsky.DreamOS
{
    public class MessagingManager : MonoBehaviour
    {
        ...
        [SerializeField] private DateAndTimeManager timeManager;
        ...
        [HideInInspector] public bool isStoryTellerOpen;
        ...
        void Awake()
        {
            if (audioManager == null && FindObjectsOfType(typeof(AudioManager)).Length > 0) { audioManager = (AudioManager)FindObjectsOfType(typeof(AudioManager))[0]; }
            ...
            if (useNotifications && notificationManager == null && FindObjectsOfType(typeof(NotificationManager)).Length > 0) { notificationManager = (NotificationManager)FindObjectsOfType(typeof(NotificationManager))[0]; }
            ...
            for (int i = 0; i < chatList.Count; ++i)
            {
                ...
                for (int x = 0; x < chatList[i].chatAsset.messageList.Count; ++x)
                {
                    ...
                    else if (chatList[i].chatAsset.messageList[x].objectType == MessagingChat.ObjectType.AudioMessage)
                    {
                        ...
                        audioMessage.aSource = audioManager.audioSource;
        ...
        void CreateCustomMessageFromInput(ChatLayoutPreset parent, bool isSelf)
        {
            ...
            if (audioManager != null && !sentSoundHelper) 
            { 
                audioManager.audioSource.PlayOneShot(sentMessageSFX);
            }
        ...
        public void CreateIndividualMessage(ChatLayoutPreset parent, string msgContent, string locKey = null)
        {
            ...
            else if (audioManager != null) { audioManager.audioSource.PlayOneShot(receivedMessageSFX); }
        }
        ...
        public void CreateCustomMessage(ChatLayoutPreset parent, string message, string time, string locKey = null)
        {
            ...
            if (audioManager != null) { audioManager.audioSource.PlayOneShot(sentMessageSFX); }
        ...
        public void CreateDynamicMessage(int layoutIndex, bool waitingForTimer = true)
        {
            ...
            if (!waitingForTimer && chatList[layoutIndex].chatAsset.useDynamicMessages && chatList[layoutIndex].chatAsset.dynamicMessages[dynamicMessageIndex].messageContent == latestDynamicMessage)
            {
                GameObject msgObj = Instantiate(textMessageRecieved, new Vector3(0, 0, 0), Quaternion.identity);
                ...
                if (useNotifications && !layout.gameObject.activeInHierarchy || useNotifications && selectedLayout.name != layout.gameObject.name)
                {
        ...
        public string GetTimeData()
        {
            ...
            if (timeManager != null && timeManager.useShortTimeFormat)
            {
                if (timeManager.currentMinute.ToString().Length == 1) { tempValue = timeManager.currentHour + ":" + "0" + timeManager.currentMinute; }
                else { tempValue = timeManager.currentHour + ":" + timeManager.currentMinute; }

                if (timeManager.isAm) { tempValue = tempValue + " AM"; }
                else { tempValue = tempValue + " PM"; }
            }

            else if (timeManager != null && !timeManager.useShortTimeFormat)
            {
                if (timeManager.currentMinute.ToString().Length == 1) { tempValue = timeManager.currentHour + ":" + "0" + timeManager.currentMinute; }
                else { tempValue = timeManager.currentHour + ":" + timeManager.currentMinute; }
            }
        ...
        {
            notificationManager.CreatePopupNotification(icon, name, description, true, receivedMessageSFX);
        }
